...
function withTenantRole(user) {
  ...
  return {
    ...user,
    user_metadata: { ...(user.user_metadata || {}), role, tenant: TENANT },
  };
}
...
export async function createUser({ name, email, password, role = "user" }) {
  ...
  if (existing) {
    if (roleForTenant(existing)) throw new Error("Este e-mail ja possui acesso ao BI da Florybal.");
...
export async function loginUser({ email, password }) {
  ...
  if (!scopedUser) throw new Error("Este acesso nao pertence ao BI da Florybal.");
