...
function importResponseSummary(parsedPayload, files) {
  ...
  const reportPeriods = [...new Set((parsedPayload.reportImports || []).map((item) => item.period?.key).filter(Boolean))].sort();
  ...
  return {
    ...
    periods: reportPeriods.length ? reportPeriods : (parsedPayload.periods || []),
