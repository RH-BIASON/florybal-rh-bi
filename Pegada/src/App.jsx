...
function buildAnalytics(rows, dataset = {}, filterContext = {}) {
  ...
  const scheduleRows = (dataset.vacationSchedule || []).filter((item) => branchAllowed(item) && queryAllowed(item));
  ...
  const scheduleReportPositionDate = currentSchedule
    .map((item) => item.period?.positionDate || item.period?.end)
    .filter(Boolean)
    .sort()
    .at(-1) || null;
  const referenceDate = businessToday();
  ...
  const vacationSchedule = Array.from(scheduleByPerson.values())
    .map((item) => {
      const classification = classifyVacationUrgency(item.deadline, item.balanceDays, referenceDate);
  ...
  return (
    <section className="grid two">
      <Panel
        ...
        icon={CalendarDays}
        wide
        subtitle={`Horizonte do relatório até ${shortDate(analytics.scheduleReportPositionDate)} · urgência calculada em ${shortDate(analytics.scheduleReferenceDate)} · período aquisitivo mais antigo por colaborador.`}
      >
...
function importSummaryFromDataset(payload, files) {
  ...
  const reportPeriods = [...new Set((payload.reportImports || []).map((item) => item.period?.key).filter(Boolean))].sort();
  return {
    files: files.map((file) => file.name),
    reports: payload.reportImports || [],
    periods: reportPeriods.length ? reportPeriods : (payload.periods || []),
