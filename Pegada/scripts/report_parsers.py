...
def period_from_report(text, kind):
    ...
    if kind == "vacation_schedule":
        period = period_from_date(match.group(1))
        if period:
            period["positionDate"] = iso_date(match.group(1))
        return period
    return period_from_date(match.group(1))


def branch_info(code, printed_name=""):
    branch_code = str(code).zfill(4)
    item = DIRECTORY["establishments"].get(branch_code)
    if not item:
        return None
    name = item.get("shortName") or printed_name or item["name"]
    return {
        "code": branch_code,
        "internalCode": item.get("internalCode", ""),
        "name": name,
        "label": f"{branch_code} - {name}",
...
def page_branch(page):
    ...
    for index, line in enumerate(lines[:15]):
        if re.fullmatch(r"\d{4}", line) and index + 1 < len(lines):
            info = branch_info(line, lines[index + 1])
            return {**info, "reportName": lines[index + 1]}
        match = re.match(r"^(\d{4})\s*(?:-\s*)?(.+)$", line)
        if match:
            info = branch_info(match.group(1), match.group(2))
            return {**info, "reportName": match.group(2)}
    return None


def public_branch(branch):
    return {key: value for key, value in branch.items() if key != "reportName"}
...
def parse_provision(path, kind):
    ...
    for page_number, page in enumerate(doc, 1):
        ...
        for row in page_rows:
            ...
            if not re.fullmatch(r"\d{1,10}", contract) or not re.search(r"[A-Za-zÀ-ÿ]", name):
                continue
            ...
            if current is None or inss is None or fgts is None:
                ...
                continue
            clean_branch = public_branch(branch)
            records.append({
                ...
                "period": period,
                "company": clean_branch["company"],
                "cnpj": clean_branch["cnpj"],
                "branch": clean_branch,
                "contract": contract,
                "name": name,
                "basis": basis or 0,
        ...
        branch_name = re.sub(r"\s+", "", normalized(branch.get("reportName") or branch["name"]))
        branch_rows = [
            row
            for row in page_rows
            if "NO CONTR.:" in normalized(row["text"])
            and re.sub(r"\s+", "", normalized(row["text"])).startswith(branch_name)
        ]
        if branch_rows:
            row = branch_rows[-1]
            current = value_in_x(row, 400, 445)
            ...
            if None not in (current, inss, fgts):
                summaries.append({"reportType": kind, "sourceFile": path.name, "sourcePage": page_number, "period": period, "branch": public_branch(branch), "isGrandTotal": False, "currentBalance": current, "inss": inss, "fgts": fgts, "total": round(current + inss + fgts, 2)})
    last_rows = rows_by_y(doc[-1])
    company_rows = [row for row in last_rows if "CALCADOS PEGADA NORD" in normalized(row["text"]) and "CONTR" in normalized(row["text"])]
    if company_rows:
        row = company_rows[-1]
        current = value_in_x(row, 400, 445)
        inss = value_in_x(row, 590, 630)
...
def schedule_row(row, employee, branch, period, source_file, page_number):
    start = text_in_x(row, 250, 307)
    end = text_in_x(row, 307, 370).replace("a ", "").strip()
    if not DATE_RE.fullmatch(start) or not DATE_RE.fullmatch(end):
        return None
    absences = br_number(text_in_x(row, 375, 414))
    lost = br_number(text_in_x(row, 414, 454))
    taken = br_number(text_in_x(row, 454, 502))
    bonus = br_number(text_in_x(row, 502, 546))
    balance = br_number(text_in_x(row, 546, 584))
    deadline = text_in_x(row, 584, 642)
    scheduled = text_in_x(row, 642, 710)
    scheduled_days = br_number(text_in_x(row, 710, 746))
    clean_branch = public_branch(branch)
    return {
...
def parse_vacation_schedule(path):
    ...
    for page_number, page in enumerate(doc, 1):
        ...
        for row in rows_by_y(page):
            contract = text_in_x(row, 45, 90)
            name = text_in_x(row, 90, 250)
            if re.fullmatch(r"\d{1,10}", contract) and re.search(r"[A-Za-zÀ-ÿ]", name):
