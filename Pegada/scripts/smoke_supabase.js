...
const bucket = process.env.PEGADA_SUPABASE_STORAGE_BUCKET || "pegada-payroll-pdfs";
...
const employee = {
  id: "2026-05-000-999",
  sourceFile: "SYNTHETIC-PEGADA.pdf",
  sourcePage: 1,
  period: { key: "2026-05", label: "05/2026", start: "2026-05-01", end: "2026-05-31" },
  branch: { code: "0005", name: "MATRIZ", label: "0005 - MATRIZ" },
  contract: "999",
...
const file = {
  path: tempPdf,
  originalname: "SYNTHETIC-PEGADA.pdf",
  filename: `${importId}-SYNTHETIC-PEGADA.pdf`,
...
try {
  ...
  const rows = await supabaseFetch(`/rest/v1/pegada_payroll_imports?id=eq.${encodeURIComponent(importId)}&select=id,status,employee_records`);
  if (rows?.[0]?.status !== "imported" || rows?.[0]?.employee_records !== 1) {
    throw new Error("Smoke test gravou dados inesperados.");
  }
  const fileRows = await supabaseFetch(`/rest/v1/pegada_payroll_import_files?import_id=eq.${encodeURIComponent(importId)}&select=storage_path`);
  await supabaseFetch(`/rest/v1/pegada_payroll_imports?id=eq.${encodeURIComponent(importId)}`, { method: "DELETE" });
  await Promise.all(
    (fileRows || []).map((row) =>
