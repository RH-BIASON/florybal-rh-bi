...
const DEFAULT_BUCKET = "pegada-payroll-pdfs";
const TABLES = {
  imports: "pegada_payroll_imports",
  records: "pegada_payroll_records",
  events: "pegada_payroll_events",
  audits: "pegada_payroll_audit_results",
  files: "pegada_payroll_import_files",
};
...
function config() {
  ...
  const bucket = process.env.PEGADA_SUPABASE_STORAGE_BUCKET || DEFAULT_BUCKET;
...
async function patchImport(id, fields) {
  return requestJson(`/rest/v1/${TABLES.imports}?id=eq.${encodeURIComponent(id)}`, {
    method: "PATCH",
    headers: {
      "Content-Type": "application/json",
...
async function uploadActiveDataset(payload) {
  const { url, bucket } = config();
  const bytes = gzipSync(Buffer.from(JSON.stringify(payload), "utf8"), { level: 9 });
  const storagePath = "active/payroll.json.gz";
...
async function downloadActiveDataset() {
  const { url, bucket } = config();
  const response = await fetch(`${url}/storage/v1/object/${bucket}/active/payroll.json.gz`, {
    method: "GET",
    headers: headers(),
  });
...
export async function saveImportToSupabase(id, status, files, payload = null, detail = "") {
  ...
  await insertRows(TABLES.imports, [
    imported
      ? {
          ...importRow,
          status: "failed",
  ...
  try {
    if (files.length) {
      ...
      await insertRows(TABLES.files, fileRows);
    }

    if (imported && payload) {
      await uploadActiveDataset(payload);
      if (process.env.SUPABASE_WRITE_ANALYTICS_TABLES === "true") {
        await insertRows(TABLES.records, payload.employees.map((employee) => recordRow(id, employee)));
        await insertRows(TABLES.events, payload.employees.flatMap((employee) => eventRows(id, employee)));
      }
      await insertRows(TABLES.audits, [
        {
          import_id: id,
...
export async function latestPayrollFromSupabase() {
  ...
  const activeDataset = await downloadActiveDataset();
  if (activeDataset) return activeDataset;
  const rows = await requestJson(`/rest/v1/${TABLES.imports}?status=eq.imported&select=dataset&order=imported_at.desc&limit=1`, {
    method: "GET",
  });
...
export async function importHistoryFromSupabase() {
  ...
  const rows = await requestJson(
    `/rest/v1/${TABLES.imports}?select=id,status,imported_at,source_files,detail,periods,branch_count,employee_records,reconciliation_matched,diagnostic_count,unclassified_event_count&order=imported_at.desc&limit=80`,
    { method: "GET" },
  );
