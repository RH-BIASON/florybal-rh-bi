...
export function normalizeVacationScheduleCompetence(dataset) {
  ...
  const reportImports = (dataset.reportImports || []).map((item) => item.reportType === "vacation_schedule"
    ...
    : item);
  const periodRecords = [...(dataset.employees || []), ...(dataset.provisions || [])];
  return {
    ...dataset,
    vacationSchedule,
    reportImports,
    periods: sortPeriods(periodRecords.map((item) => item.period?.key).filter(Boolean)),
  };
}
...
export function mergePayrollDatasets(baseDataset, importedDataset) {
  ...
  const periods = sortPeriods([...employees, ...provisions].map((item) => item.period?.key).filter(Boolean));
